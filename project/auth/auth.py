...
def login():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        select_all_query = 'SELECT * FROM accounts WHERE username  = %s AND password = %s', (
            username, password)
        ...
        if account is not None:
            ...
            select_id_query = 'SELECT id FROM accounts WHERE username  = %s AND password = %s', (
                username, password)

            session['id'] = user_exist(
                connection, select_id_query, username, password)
            print('Session id', session['id'])
            select_username_query = 'SELECT username FROM accounts WHERE username  = %s AND password = %s', (
                username, password)
            session['username'] = user_exist(
                connection, select_username_query, username, password)
